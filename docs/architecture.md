...
Endpointy listy, zakupów i przepisów pod `kitchens/:kitchenId`. Kategorie: `…/recipe-categories`. Import z URL: `POST …/recipes/import/preview` (sesja + członkostwo; pobieranie HTTPS z walidacją SSRF wg OWASP — DNS A/AAAA, blokada prywatnych/metadanych IP, ręczne przekierowania max 3, limity czasu/rozmiaru/rate; bez cookies aplikacji; HTML i JSON-LD jako niezaufane). Lista przepisów filtruje w API (`categoryIds` OR, `uncategorized`, `search`) przy zachowaniu reguł widoczności. Każda operacja wymaga członkostwa w kuchni; prywatne przepisy tylko dla autora. Lookup wartości odżywczych: `GET …/nutrition-lookups/by-ean` (Open Food Facts przez NestJS; timeout, cache, obsługa 429/503; testy CI używają lokalnego mocka HTTP).
