...
- `Product` (`normalizedName`, unikalność `(kitchenId, normalizedName)`, `defaultUnit`, opcjonalne `ean` / `imageUrl` / `imageMediaId` / `category`, opcjonalne `groupId` / `brand` / `variantLabel` / `packageQuantity`+`packageUnit` (`PackageContentUnit`: piece|gram|kilogram|milliliter|liter), `ProductPurchaseOption` — warianty zakupu z jednym domyślnym),
- `ProductGroup` (opcjonalny rodzaj w kuchni, np. „Mozzarella”: `normalizedName`, unikalność `(kitchenId, normalizedName)`; produkty mogą mieć `groupId` z `ON DELETE SET NULL` — usunięcie grupy nie kasuje produktów),
...
- `StockItem` (`initialQuantity`, `quantity`, opcjonalne `purchasePriceMinor`, `currency`, opcjonalne `storeName` dla partii bez `Purchase`, miejsce, daty, opcjonalne `ean` / `imageUrl`, powiązanie 1:1 z `PurchaseLineItem` gdy partia powstała z checkoutu). Ilości: `DECIMAL(12,3)`.
- `ProductIntakeIdempotency` (unikalny `idempotencyKey`, `kitchenId`, `resultJson` — atomowe przyjęcie produktu z opcjonalnym zapasem i nutrition),
