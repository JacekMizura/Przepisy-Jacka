...
Nazwy produktów są unikalne w kuchni po normalizacji (trim, lowercase, zbiciu spacji). Wyświetlana pozostaje oryginalna `name`. Produkt można **zarchiwizować** (`archivedAt`) zamiast usuwać: znika z aktywnego katalogu i selektorów, ale partie, zakupy, paragony, zużycia, koszty, zdjęcia i powiązania z przepisami zostają. Przy dodatnim stanie widać go w zapasach z oznaczeniem „Zarchiwizowany”. Oczekująca pozycja listy zakupów blokuje archiwizację (409). Przywrócenie wraca do katalogu. Ponowne utworzenie tej samej nazwy przy zarchiwizowanym produkcie zwraca kontrolowany 409 z możliwością przywrócenia. Trwałe usunięcie (`?permanent=true`) tylko dla produktu nigdy nieużytego.
