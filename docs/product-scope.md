...
Ten sam produkt z katalogu może mieć wiele partii (np. zakupy w różnych sklepach). W widoku zbiorczym zapasów grupujemy po `productId` — jedna pozycja na produkt z łączną ilością i rozwinięciem partii (sklep, data, termin ważności, cena opakowania/razem, odnośnik do zakupu). Produkty przypisane do tego samego `ProductGroup` (rodzaju) są na liście zapasów prezentowane jako jeden zwarty wiersz rodzaju z zagnieżdżonymi wariantami (bez zmiany modelu danych). Listy `stock-summary` i `catalog` są paginowane po stronie API (`page`/`limit`, domyślnie 50, max 100) z filtrami i sortowaniem w zapytaniu — nie pobieramy tysięcy rekordów do przeglądarki. Widok Zapasy pokazuje wyłącznie dodatni stan (w tym zarchiwizowane z pozostałą ilością); Katalog obejmuje produkty ze stanem i bez. Ilości w UI mogą być skalowane prezentacyjnie (np. `2400 g` → `2,4 kg`). Nie scalamy automatycznie produktów o podobnych nazwach.
...
Nazwy produktów są unikalne w kuchni po normalizacji (trim, lowercase, zbiciu spacji). Wyświetlana pozostaje oryginalna `name`. Produkt można **zarchiwizować** (`archivedAt`) zamiast usuwać: znika z aktywnego katalogu i selektorów, ale partie, zakupy, paragony, zużycia, koszty, zdjęcia i powiązania z przepisami zostają. Przy dodatnim stanie widać go w zapasach z oznaczeniem „Zarchiwizowany”. Oczekująca pozycja listy zakupów blokuje archiwizację (409). Przywrócenie wraca do katalogu. Ponowne utworzenie tej samej nazwy przy zarchiwizowanym produkcie zwraca kontrolowany 409 z możliwością przywrócenia. Omyłkowe przyjęcie produktu (ręczne partie + nutrition, bez zakupów/zużyć/przepisów) cofa się przez `GET …/products/:id/removal-preview` + `POST …/undo-addition` — nie przez `?permanent=true`. Trwałe usunięcie (`?permanent=true`) pozostaje tylko jako fallback dla produktu nigdy nieużytego bez zapasu i nutrition.
...
API zwraca dostępność składników względem zapasów (`available`, `partial`, `missing`, `unknown`) z bezpiecznym skalowaniem porcji oraz przeliczeniami g/kg i ml/l. Dla braków proponuje pełne opakowania (`ceil(brak / zawartość)`). Braki można dodać do istniejącej listy zakupów z wyborem wariantu i idempotencją. Etapy przepisu mogą mieć opcjonalny tytuł, wskazówkę i czas trwania. Składniki potrzebne w konkretnym kroku są osobną relacją wiele-do-wielu — nie wynikają z interpretacji tekstu instrukcji. Opcjonalny **plan przygotowania** (`preparationPlanEnabled`) dodaje zależności między krokami, czas pracy/oczekiwania i timery dla osobnego pełnoekranowego trybu `/cook`. Nie zastępuje tradycyjnego widoku ani wysuwanego Asystenta gotowania.
