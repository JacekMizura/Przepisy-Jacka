...
- `apps/web` — jasny sidebar (emerald), zapasy z **kompaktowymi kartami** (object-contain, badge kategorii z ikoną, data ważności tylko gdy jest, koszt z partii gdy znany), katalog jako tabela z **rozwijalnymi wariantami**, lista zakupów z jasnym podsumowaniem; dodawanie do listy respektuje `purchaseMode` packaged (`purchaseOptionId` + `packageCount`); filtry/paginacja/API bez zmian; względne `/api/*` przez proxy,
