...
- `apps/web` — layout **ciemny sidebar** (Moja Kuchnia / zapasy / baza produktów / lista zakupów / historia / przepisy / domownicy), logowanie, rejestracja, kuchnie, zaproszenia; zapasy w zakładkach URL (`?view=stock|catalog|history`) z **kartami fotograficznymi** (siatka do 4 kolumn, status terminu, STAN/MIEJSCE/DATA, Zużyj + Dodaj partię), **tabelą katalogu** (EAN, jednostka), **listą zakupów** z ciemnym „Modułem finansowym”; filtry/paginacja/API bez zmian; dialog zużycia, grupy produktów, przepisy, media i nutrition jak wcześniej; względne `/api/*` przez proxy,
