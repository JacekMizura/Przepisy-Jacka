...
- `apps/web` — layout sidebar (Moja Kuchnia / zapasy / lista zakupów / historia zakupów / przepisy / domownicy), logowanie, rejestracja, kuchnie, zaproszenia, zapasy w trzech zakładkach URL (`?view=stock|catalog|history`) z **nowoczesną siatką kart** zapasów (1/2/3 kolumny, limit 24) oraz **kartami katalogu** (do 4 kolumn, `CatalogCard`), paginacją API, połączonym paskiem zakładek+wyszukiwania+filtrów, filtrami w URL i menu `…` przez portal; dialog zużycia oraz cofnięciem; **lista zakupów** w stylu makiety (`#009060`, pasek postępu, sekcje Do kupienia/Kupione + sticky Podsumowanie, modal dodawania); **UX przyjęcia produktu** (`/products/new`, `/edit`, `/add-batch`) — create 1:1 z kafelkami kategorii, **jednym miejscem trybu zakupu** (`W opakowaniach` / `Na wagę / luzem`, bez duplikatu w Zapasy), comboboxem sklepu, ceną za opakowanie lub łączną i snapshotem partii; prezentacja zapasów bez ceny za gram; **grupy produktów** jako jedna karta rodzaju z wierszami wariantów (bez Zużyj na grupie; zdjęcia tylko przy wariantach; ikona rodzaju zamiast kolażu); przepisy, media i nutrition jak wcześniej; względne `/api/*` przez proxy,
