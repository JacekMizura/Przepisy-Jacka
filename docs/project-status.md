...
- `apps/web` — layout sidebar (Moja Kuchnia / zapasy / lista zakupów / historia zakupów / przepisy / domownicy), logowanie, rejestracja, kuchnie, zaproszenia, zapasy w trzech zakładkach URL (`?view=stock|catalog|history`) z poprawionym układem filtrów; dialog zużycia oraz cofnięciem; **UX przyjęcia produktu** (`/products/new`, `/edit`, `/add-batch`) — create 1:1 z kafelkami kategorii, **comboboxem sklepu**, ceną za opakowanie i snapshotem partii (`packageCount` / quantity / unit); prezentacja zapasów bez ceny za gram; **grupy produktów** jako jedna karta rodzaju (stabilna miniatura 2×2, zagnieżdżone warianty/partie, ilości typu `2,4 kg`, usuwanie partii w menu); uproszczony wybór `W opakowaniach` / `luzem` bez edytora wielu profili; katalog, przepisy, media i nutrition jak wcześniej; względne `/api/*` przez proxy,
