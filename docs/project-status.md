...
- `apps/web` — layout sidebar (Moja Kuchnia / zapasy / lista zakupów / historia zakupów / przepisy / domownicy), logowanie, rejestracja, kuchnie, zaproszenia, zapasy w trzech zakładkach URL (`?view=stock|catalog|history`) z kompaktową tabelą (~56–64 px), paginacją API (`stock-summary` / `catalog` → `{ items, page, limit, total, pageCount }`), filtrami w URL (`q`, `category`, `place`, `unit`, `expiry`, `archived`, `sort`, `page`) i menu `…` przez portal; dialog zużycia oraz cofnięciem; **UX przyjęcia produktu** (`/products/new`, `/edit`, `/add-batch`) — create 1:1 z kafelkami kategorii, **jednym miejscem trybu zakupu** (`W opakowaniach` / `Na wagę / luzem`, bez duplikatu w Zapasy), comboboxem sklepu, ceną za opakowanie lub łączną i snapshotem partii; prezentacja zapasów bez ceny za gram; **grupy produktów** jako jeden zwarty wiersz rodzaju (bez Zużyj na grupie; zdjęcia tylko przy wariantach); katalog, przepisy, media i nutrition jak wcześniej; względne `/api/*` przez proxy,
