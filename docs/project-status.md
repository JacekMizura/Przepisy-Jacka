...
- `apps/web` — jasny sidebar (emerald), zapasy z **kompaktowymi kartami poziomymi** (kwadratowe zdjęcie `object-contain` bez przycinania, badge kategorii z ikoną, data ważności tylko gdy jest, koszt z partii gdy znany; podpisane URL-e z `stock-summary`), katalog jako tabela z **rozwijalnymi wariantami**, lista zakupów bez szarych ramek miniatur, z **wyraźną ilością do kupienia** i szacunkiem ceny (`estimatedPriceMinor` z ostatniego zakupu) oraz modalem „tylko całe opakowania” dla `purchaseMode=packaged`; filtry/paginacja/API bez zmian; względne `/api/*` przez proxy,
