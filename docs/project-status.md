...
- `apps/web` — layout sidebar (Moja Kuchnia / zapasy / lista zakupów / historia zakupów / przepisy / domownicy), logowanie, rejestracja, kuchnie, zaproszenia, zapasy w trzech zakładkach URL (`?view=stock|catalog|history`) z poprawionym układem filtrów; dialog zużycia oraz cofnięciem; **UX przyjęcia produktu** (`/products/new`, `/edit`, `/add-batch`) — create 1:1 z kafelkami kategorii, **jednym miejscem trybu zakupu** (`W opakowaniach` / `Na wagę / luzem`, bez duplikatu w Zapasy), comboboxem sklepu, ceną za opakowanie lub łączną i snapshotem partii; prezentacja zapasów bez ceny za gram; **grupy produktów** jako jedna karta rodzaju (ikona grupy bez kolażu zdjęć; zdjęcia tylko przy wariantach); menu `…` przez portal nad kartami; katalog, przepisy, media i nutrition jak wcześniej; względne `/api/*` przez proxy,
