...
- `apps/web` — layout sidebar (Moja Kuchnia / zapasy / lista zakupów / historia zakupów / przepisy / domownicy), logowanie, rejestracja, kuchnie, zaproszenia, zapasy w trzech zakładkach URL (`?view=stock|catalog|history`) z **nowoczesną siatką kart** (`rounded-3xl`, 1/2/3 kolumny, limit 24/stronę), paginacją API (`stock-summary` / `catalog` → `{ items, page, limit, total, pageCount }`), połączonym paskiem zakładek+wyszukiwania+filtrów, filtrami w URL (`q`, `category`, `place`, `unit`, `expiry`, `archived`, `sort`, `page`) i menu `…` przez portal; dialog zużycia oraz cofnięciem; **UX przyjęcia produktu** (`/products/new`, `/edit`, `/add-batch`) — create 1:1 z kafelkami kategorii, **jednym miejscem trybu zakupu** (`W opakowaniach` / `Na wagę / luzem`, bez duplikatu w Zapasy), comboboxem sklepu, ceną za opakowanie lub łączną i snapshotem partii; prezentacja zapasów bez ceny za gram; **grupy produktów** jako jedna karta rodzaju z wierszami wariantów (bez Zużyj na grupie; zdjęcia tylko przy wariantach; ikona rodzaju zamiast kolażu); katalog, przepisy, media i nutrition jak wcześniej; względne `/api/*` przez proxy,
