...
- `apps/mobile` — Expo Router: logowanie/rejestracja Better Auth (SecureStore), wybór kuchni, zakładki Zapasy / Zakupy / Przepisy (placeholder) / Więcej; zapasy z zużyciem i historią; lista zakupów, checkout z paragonem (begin→PUT R2→complete), historia zakupów; `EXPO_PUBLIC_API_URL` jako jedyna zmienna publiczna,
...
Mobile: moduł przepisów (lista/szczegóły/dostępność/braki) oraz dopracowanie zdjęć produktów na liście zapasów (join z katalogiem produktów).
