...
- `apps/web` — layout sidebar (Moja Kuchnia / zapasy / lista zakupów / historia zakupów / przepisy / domownicy), logowanie, rejestracja, kuchnie, zaproszenia, zapasy, przepisy (kafelki z okładkami, tworzenie, szczegóły w układzie editorialnym: hero, sticky składniki, narracyjne kroki, kopiowanie składników, print CSS, dostępność oraz koszt i makro, edycja); zdjęcia produktów, okładek i kroków przepisu przez magazyn mediów (`media-upload.ts` + `MediaImageField`), wartości odżywcze produktu w tworzeniu i edycji katalogu (OFF po EAN); względne `/api/*` przez serwerowy proxy do `API_ORIGIN`,
