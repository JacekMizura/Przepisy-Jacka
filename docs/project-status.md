...
- `apps/web` — jasny sidebar (emerald), zapasy z **kompaktowymi kartami poziomymi** (kwadratowe zdjęcie `object-cover`, badge kategorii z ikoną, data ważności tylko gdy jest, koszt z partii gdy znany; podpisane URL-e z `stock-summary`), katalog jako tabela z **rozwijalnymi wariantami**, lista zakupów bez szarych ramek miniatur, ze **szacunkiem ceny** (`estimatedPriceMinor` z ostatniego zakupu) i modalem „tylko całe opakowania” dla `purchaseMode=packaged`; filtry/paginacja/API bez zmian; względne `/api/*` przez proxy,
