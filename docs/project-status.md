...
- `apps/mobile` — Expo Router: logowanie/rejestracja Better Auth 1.7.2 (SecureStore), wybór kuchni, zakładki Zapasy / Zakupy / Przepisy (placeholder) / Więcej; zapasy z zużyciem i **odpisem** (`kind`/`reason` w `StockConsumption`), historią oraz zdjęciem produktu; lista zakupów, checkout z paragonem; `EXPO_PUBLIC_API_URL`; `AUTH_TRUSTED_ORIGINS` z dokładnym `mojakuchnia://`,
