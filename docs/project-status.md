...
- `apps/mobile` — Expo Router: logowanie/rejestracja Better Auth (SecureStore), wybór kuchni, zakładki Zapasy / Zakupy / Przepisy (placeholder) / Więcej; zapasy z zużyciem/odpisem i historią oraz zdjęciem produktu; lista zakupów, checkout z paragonem (begin→PUT R2→complete), historia zakupów; `EXPO_PUBLIC_API_URL` jako jedyna zmienna publiczna; `AUTH_TRUSTED_ORIGINS` z dokładnym `mojakuchnia://` (bez exp:// / wildcardów),
